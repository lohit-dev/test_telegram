...
export class AuthHandler {
   ...
   */
  static async register(telegramId: string, password: string) {
    try {
      // Check if user already exists
      const existingUser = await UserService.authenticateUser(telegramId, password);
      if (existingUser) {
        return 'You are already registered. Use /login to access your wallets.';
      }
      
      // Register the user
      await UserService.registerUser(telegramId, password);
   ...
   * @returns Success message or error
   */
  static async login(telegramId: string, password: string) {
    try {
      ...
      activeSessions.set(telegramId, password);
   ...
   */
  static logout(telegramId: string) {
   ...
   */
  static isAuthenticated(telegramId: string) {
    return activeSessions.has(telegramId);
  }
   ...
   */
  static getPassword(telegramId: string) {
   ...
   * @returns Success message or error
   */
  static async createWallet(telegramId: string, chain: any, starknetService: StarknetService) {
    try {
      ...
      // Create wallets
      const walletResponse = await WalletService.createWallets(chain, starknetService);
   ...
   */
  static async getWallets(telegramId: string) {
    try {
      // Check if user is authenticated
      const password = this.getPassword(telegramId);
      if (!password) {
      ...
      // Format wallet list
      const walletList = wallets.map((wallet: any, index: number) => {
   ...
   */
  static async getWalletDetails(telegramId: string, walletIndex: number) {
    try {
      // Check if user is authenticated
      const password = this.getPassword(telegramId);
      if (!password) {
      ...
      const wallet = await DbWalletService.getWallet(user.id, walletInfo.id, password);
...
Use /show_mnemonic ${walletIndex} to reveal the mnemonic (if available).`;
   ...
   */
  static async showPrivateKey(telegramId: string, walletIndex: number) {
    try {
      // Check if user is authenticated
      const password = this.getPassword(telegramId);
      if (!password) {
